...
use crate::{
	...
	Config, Error, PriceGetter, BuyoutLimit, Buyouts, ValidityError,
};
use frame_support::{assert_err, assert_ok, traits::UnixTime, assert_noop};
use orml_traits::MultiCurrency;
use sp_arithmetic::{FixedU128, traits::One};
use sp_runtime::{traits::BadOrigin, transaction_validity::{InvalidTransaction, TransactionValidityError}};
...
fn buyout_using_dot_given_exchange_amount_in_dot_succeeds() {
	run_test(|| {
		...
		let initial_treasury_native_balance = get_free_balance(GetNativeCurrencyId::get(), &TreasuryAccount::get());
        ...
        let basic_asset_price = <OracleMock as PriceGetter<CurrencyIdOf<Test>>>::get_price::<FixedU128>(GetNativeCurrencyId::get()).expect("This is mocked so it should not fail");
        let exchange_asset_price = <OracleMock as PriceGetter<CurrencyIdOf<Test>>>::get_price::<FixedU128>(0u64).expect("This is mocked so it should not fail");

        // Add fee to basic asset price
        let basic_asset_price_with_fee = basic_asset_price * (FixedU128::from(SellFee::get()) + FixedU128::one());

        // Calculate Native buyout amount
        let buyout_amount = crate::Pallet::<Test>::multiply_by_rational(exchange_amount, exchange_asset_price.into_inner(), basic_asset_price_with_fee.into_inner()).expect("This is mocked so it should not fail");
        ...
        let final_treasury_native_balance = get_free_balance(GetNativeCurrencyId::get(), &TreasuryAccount::get());
...
fn buyout_using_dot_given_buyout_amount_in_native_succeeds() {
	run_test(|| {
        ...
        let exchange_amount = crate::Pallet::<Test>::multiply_by_rational(buyout_amount, basic_asset_price_with_fee.into_inner(), exchange_asset_price.into_inner()).expect("This is mocked so it should not fail");
...
fn user_update_buyout_amount_limit_fails() {
	run_test(|| {
		...
		let buyout_amount_limit = 200 * UNIT;
        assert_noop!(crate::Pallet::<Test>::update_buyout_limit(
			RuntimeOrigin::signed(user),
			Some(buyout_amount_limit.into()),
		), BadOrigin);
...
fn attempt_buyout_with_wrong_currency_fails() {
    run_test(|| {
		...
		let initial_treasury_native_balance = get_free_balance(native_currency_id, &TreasuryAccount::get());

        assert_eq!(initial_user_native_balance, USERS_INITIAL_BALANCE);
        assert_eq!(initial_treasury_native_balance, TREASURY_INITIAL_BALANCE);

		let buyout_amount = 100 * UNIT;
		assert_noop!(crate::Pallet::<Test>::buyout(
			RuntimeOrigin::signed(user),
			native_currency_id,
			Amount::Buyout(buyout_amount),
...
fn attempt_buyout_with_insufficient_treasury_balance_fails() {
    run_test(|| {
        ...
        assert_ok!(<<Test as Config>::Currency>::transfer(RuntimeOrigin::signed(TREASURY_ACCOUNT), user, native_currency_id,  TREASURY_INITIAL_BALANCE));
...
mod signed_extension {
    use frame_support::{weights::Weight, dispatch::DispatchInfo};
    ...
    fn validate_skip_other_calls_succeeds() {
        run_test(|| {
            let buyout_call =
                RuntimeCall::TreasuryBuyoutExtension(crate::Call::update_buyout_limit { limit: None });

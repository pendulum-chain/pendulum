...
| | |
|---|---|
| **Status** | Draft — technical planning document for the community discussion |
| **Date** | 2026-07-10 |
...
|---|---|---|
| Supply model on Base | Entire max issuance pre-minted to the vault at deployment; token has no mint function, no owner, no upgradeability | Correct tracker stats from day one; eliminates infinite-mint attack surface; worst case bounded by vault balance |
| **D3** — Max issuance | Exactly 150,000,000 PEN | A clean, canonical figure rather than the untidy live issuance (~149.93M, no inflation — a fee-burn artifact that keeps drifting). The rounding delta (~67k PEN, ~0.045%) can never be released by migration (no matching burns can exist), stays excluded from circulating supply, and at window close moves to the **community treasury via the governed, timelocked sweep** (D5) — allocated to no one; only a governance decision can spend it. Per-holder conversion remains exactly 1:1 |
| **D2** — Decimals on Base | 18 (scale ×10⁶ at release, in the vault only — V7) | Exact conversion (DeFi convention); no change to holder amounts or ownership share |
| Token extensions | `ERC20Permit` + `ERC20Votes` included at deployment | `ERC20Votes` cannot be retrofitted into an immutable token; required for Governor voting |
| Migration direction | One-way only (no Base → Pendulum) | Halves the attack surface; no Base-side event attestation needed |
| **D1** — Pendulum-side effect of `migrate` | Burn | Keeps the invariant `Σ burned on Pendulum = Σ releasable on Base` trivially auditable and leaves no honeypot |
...
| **D4** — Attestor set & threshold | 4 attestors, threshold **3-of-4**, initially team-operated (one per internal RPC node) | Theft requires 3 simultaneously compromised keys; releases tolerate 1 offline attestor. Pendulum has no external node operators, so the set is team-operated; organizational independence is *not* claimed — blast-radius controls (caps, monitoring, guardian pause, ≥48h timelock) and separation of duties (guardian + monitor held by non-key-holders) carry the security |
| **D5** — Migration window end policy | Earliest-close date set at deployment (`earliestSweepTimestamp`); final length fixed by the governance proposal | An earliest close is *not* an automatic sweep — moving any remainder requires a separate governance decision + timelocked execution. Analysis indicates essentially all time-locked PEN unlocks within a few months, so a window as short as ~3 months is technically sufficient ([window analysis](pen-migration-window-analysis.md)); a referendum (`vesting-manager.remove_vesting_schedule`) can force-unlock any residue and the permanent sentinel locks before close |
| **D6** — Encumbered balances policy | Only unstaked, freely transferable PEN migrates (enforced by the pallet) | Staked/vesting/governance-locked balances must be freed first; the UI surfaces this. No vesting locks should extend beyond the window — residue handled via D5's referendum path |
...
the team's internal security-review log, with all
...
|---|---|---|
| **0 — Decisions & spec** | Finalize the design decisions (§4); community discussion + formal governance proposal fixing the final window and parameters; publish tokenomics/max-issuance statement | Proposal approved |

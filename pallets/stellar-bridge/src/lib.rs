...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		fn execute_withdrawal(
			...
			let escrow_encoded = T::GatewayEscrowKeypair::get().get_public().to_encoding().clone();
		...
		fn queue_withdrawal(
			...
			let withdrawal = Withdrawal { amount, currency, pendulum_address };
		...
		fn offchain_unsigned_tx_signed_payload(
			...
			if let Some((_, res)) = signer.send_unsigned_transaction(
				...
				|payload, signature| Call::submit_deposit_unsigned_with_signed_payload {
					payload,
					signature,
				},
			) {
		...
		fn handle_new_claimable_balances(cb_list: &Vec<ClaimableBalance>) {
			...
			let seq_num =
				Self::fetch_latest_seq_no(escrow_address).map(|num| num + 1).unwrap() as i64;
			...
			for cb in cb_list {
				...
				if let Ok(Some(_trusted_asset)) = potential_trused_asset.get::<Vec<u8>>() {
